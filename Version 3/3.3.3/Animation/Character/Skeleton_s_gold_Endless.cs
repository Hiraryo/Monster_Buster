...
public class Skeleton_s_gold_Endless : MonoBehaviour {
    float move_x = -0.4f;
    Collider2D s_Object;
    ...
    int point = 50;
    int point_count;
    public static bool combo_skeleton_Endless_add4 = false;
    int stage_lv;
    ...
    void Update()
    {
        combo_skeleton_Endless_add4 = false;
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if(collision.tag == "tatumaki")
        {
            ...
            if (player_hit_f == 1 && count < 5)
            {
                Invoke("Skeske", 1);
                sound01.PlayOneShot(sound01.clip);
                combo_skeleton_Endless_add4 = true;
            }
        }
    }
    void OnTriggerStay2D(Collider2D c)
    {
        if (c.tag == "Player" && space_check_f == true || c.tag == "kobushi"/* || c.tag == "tatumaki"*/) //敵にダメージを与えた時
        {
        ...
        if (c.tag == "Invisible" || c.tag == "tatumaki_death")
        {
            //sound01.PlayOneShot(sound01.clip);
    ...
    public static bool Get_Combo_Endless4()
    {
        return combo_skeleton_Endless_add4;
    }
