...
public class Other : MonoBehaviour
{
    ...
    void Update()
    {
        if (Other_Message_f == 1)
        {
            ...
            Other_Message.transform.DOScale(endValue: new Vector2(1.0f, 0.8f), duration: 0.6f).OnComplete(() =>
            {
        ...
        if (Other_Message_f == 2)
        {
            ...
            Sequence sequence = DOTween.Sequence()
                .OnStart(() =>
                {
                    Other_Message.transform.DOScale(endValue: new Vector2(2.0f, 1.6f), duration: 0.2f);
                })
                .Join(
                DOTween.ToAlpha(() =>
                Other_UI.color,
        ...
        if(Sound_f == 1)
        {
            ...
            .OnStart(() =>
            {
                sound.transform.DOLocalMoveX(-1500f, 0.5f);
            }).Join(
                privacy.transform.DOLocalMoveX(1500f, 0.5f)
            ).Join(
                go_title.transform.DOLocalMoveX(-1500f, 0.5f)
            ).Join(
                site.transform.DOLocalMoveX(1500f, 0.5f)
            ).OnComplete(() =>
            {
    ...
    public void Sound_Back_Push()
    {
        Sound_f = 0;
        Sound_Back_f = 1;
        sound.transform.localPosition = new Vector2(-1500f, 200f);  //230(公式サイトを表示した場合のY座標)
        privacy.transform.localPosition = new Vector2(1500f, 0f);   //70(公式サイトを表示した場合のY座標)
        go_title.transform.localPosition = new Vector2(-1500f, -200f);  //-70(公式サイトを表示した場合のY座標)
        //site.transform.localPosition = new Vector2(1500f, -230f); //-230(公式サイトを表示した場合のY座標)
    }
}
